          ...
          {cart.length === 0 ? (
            ...
            <>
              {cart.map((item) => {
                ...
                const variantSalePrice = Number((item.selectedVariant?.sale_price ?? item.selected_variant?.sale_price ?? item.selectedVariant?.selling_price ?? item.selected_variant?.selling_price) || 0);
                const variantMrp = Number((item.selectedVariant?.mrp ?? item.selected_variant?.mrp ?? item.selectedVariant?.price ?? item.selected_variant?.price ?? item.variant_price ?? item.selectedVariantPrice ?? item.selected_variant_price) || 0);
                const variantPrice = variantSalePrice > 0 ? variantSalePrice : variantMrp;
                const variantOldPrice = variantMrp > variantPrice ? variantMrp : 0;
                  ...
                  <div key={item.cartItemId} className="mc-item">
                    ...
                    <div className="mc-item-info">
                      ...
                      <p className="mc-item-price">
                        ...
                        {hasVariant && variantOldPrice > 0 && (
                          <span style={{ marginLeft: 6, textDecoration: "line-through", color: "#999" }}>
                            ₹{variantOldPrice.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                          </span>
                        )}
                      </p>
                      <div className="mc-item-qty-row">
                        <button
                          type="button"
                          className="mc-qty-btn"
                          onClick={() => updateQty(item.cartItemId, Number(item.qty) - 1)}
                          aria-label="Decrease quantity"
                        >
                          -
                        </button>
                        <span className="mc-qty-value">{Number(item.qty)}</span>
                        <button
                          type="button"
                          className="mc-qty-btn"
                          onClick={() => updateQty(item.cartItemId, Number(item.qty) + 1)}
              ...
              <div className="mc-footer-inline">
                <div className="mc-subtotal-row">
                  <span className="mc-subtotal-label">Tax Breakdown</span>
                </div>
                ...
                <div className="mc-breakdown-row total">
