...
const NewArrivalProducts = ({ limit, showViewAll = true, viewAllPath = "/new-arrivals" }) => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await api.get("/users/new-arrivals");
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .na-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 16px; }
          .na-section { padding: 50px 0; }
          .na-header-left h3 { font-size: 28px; }
        }

        @media (max-width: 768px) {
          .na-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }
          .na-header-left h3 { font-size: 26px; }
          .na-section { padding: 40px 0; }
        }

        @media (max-width: 480px) {
          .na-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
          .na-container { padding: 0 10px; }
          .na-section { padding: 40px 0; }
